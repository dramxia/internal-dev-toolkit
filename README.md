...
网站注入使用登录接口原始 `accessToken` 格式；旧记录或手填 Token 默认补齐 `Bearer ` 前缀。网站同时需要 Token 和 `userInfo`（用户信息）才能恢复登录。旧记录或手填 Token 缺少用户信息时，只能复用当前网站上同一个 Token 对应的用户信息；否则会提示先点击「登录并保存」重新登录，不写入、不跳转，也不提示注入成功。修改 Token 会清除插件中原有的用户信息和来源记录，避免把旧账号信息注入新会话。网站注入失败不会撤销已保存的 Token，可修正问题后点击按钮重试。
