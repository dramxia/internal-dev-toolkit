...
公司内部开发用浏览器插件脚手架（Chrome MV3，Chrome 116+）。提供页面增强、浏览器右侧栏、设置持久化、消息通信以及租户用户一键快捷登录，可作为内部工具开发的起点。
...
- **查询状态持久化**：两条一键登录路径的搜索条件、分页结果和已选关系按项目同时保存，关闭并重新打开侧边栏后自动恢复；AI token 等运行时凭据不会写入查询快照。
...
在浏览器右侧栏中展开「⚡ 一键快捷登录」面板后，先选择目标环境，再选择查询路径：
