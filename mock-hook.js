...
(() => {
  ...
  function findMatchingRule(url, method) {
    return mockRules.find(rule => {
      ...
      if (!rule.url.includes('*')) return false;
      try {
        const pattern = rule.url.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*');
  ...
  window.fetch = async function(input, init) {
    ...
    if (rule && rule.enabled && rule.mockMode === 'response') {
      ...
      recordRequest(url, method, init?.body, rule.mockData, 200);
      return mockResponse;
    }
    ...
    xhr.send = function(body) {
      ...
      if (rule && rule.enabled && rule.mockMode === 'response') {
        ...
        setTimeout(() => {
          ...
          recordRequest(url, method, requestBody, rule.mockData, 200);
